...
class KPIRecord(SQLModel, table=True):
    ...
    __table_args__ = (
        # Composite index speeds up the monthly recalculation and summary queries
        # that filter by department + date + metric_name (+ subtype).
        Index("ix_kpirecord_dept_date_metric_subtype", "department", "date", "metric_name", "subtype"),
    )
...
class ChatMessage(SQLModel, table=True):
    """Targeted message from an Admin to a specific user (or broadcast to all / a department)."""
    id: Optional[int] = Field(default=None, primary_key=True)
    ...
    department: Optional[str] = Field(default=None, index=True)  # None = global broadcast or direct message; e.g. "OHS" = department-scoped broadcast
